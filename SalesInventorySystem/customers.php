...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        $db = db();
        if ($_POST['action'] === 'add') {
            $db->prepare("INSERT INTO customers (name, email, phone, address) VALUES (?,?,?,?)")->execute([$_POST['name'], $_POST['email'], $_POST['phone'], $_POST['address']]);
            showMessage('Customer added');
        }
        if ($_POST['action'] === 'edit') {
            $db->prepare("UPDATE customers SET name=?, email=?, phone=?, address=? WHERE id=?")->execute([$_POST['name'], $_POST['email'], $_POST['phone'], $_POST['address'], $_POST['id']]);
            showMessage('Customer updated');
        }
        if ($_POST['action'] === 'delete') {
            $db->prepare("DELETE FROM customers WHERE id=?")->execute([$_POST['id']]);
...
$customers = db()->query("SELECT c.*, COUNT(so.id) as orders, COALESCE(SUM(so.total_amount),0) as spent FROM customers c LEFT JOIN sales_orders so ON c.id = so.customer_id AND so.status='completed' GROUP BY c.id ORDER BY c.name")->fetchAll();
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        ...
        .btn { background: #667eea; color: white; padding: 10px 20px; border: none; border-radius: 6px; cursor: pointer; }
        ...
        th { background: #f8fafc; font-size: 12px; color: #666; }
...
<body>
    ...
    <div class="main">
        <div class="header"><h1>Customers</h1><button class="btn" onclick="document.getElementById('addModal').classList.add('active')"><i class="fas fa-plus"></i> Add</button></div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= h($c['phone'] ?? '-') ?></td>
                            <td><?= $c['orders'] ?></td>
                            <td><?= formatPeso($c['spent']) ?></td>
                            <td>
                                <button class="btn btn-sm" onclick='editCustomer(<?= json_encode($c) ?>)'>Edit</button>
    ...
    <div class="modal" id="addModal">
        <div class="modal-content">
            <div class="modal-header"><h2>Add Customer</h2><button onclick="document.getElementById('addModal').classList.remove('active')">&times;</button></div>
            <form method="POST"><input type="hidden" name="action" value="add">
                <div class="form-group"><label>Name</label><input type="text" name="name" required></div>
                <div class="form-group"><label>Email</label><input type="email" name="email"></div>
                <div class="form-group"><label>Phone</label><input type="text" name="phone"></div>
                <div class="form-group"><label>Address</label><textarea name="address" rows="2" style="width:100%;padding:10px;border:1px solid #ddd;border-radius:6px"></textarea></div>
    ...
    <div class="modal" id="editModal">
        <div class="modal-content">
            <div class="modal-header"><h2>Edit Customer</h2><button onclick="document.getElementById('editModal').classList.remove('active')">&times;</button></div>
            <form method="POST"><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit_id">
                ...
                <div class="form-group"><label>Address</label><textarea name="address" id="edit_address" rows="2" style="width:100%;padding:10px;border:1px solid #ddd;border-radius:6px"></textarea></div>
    ...
    <script>function editCustomer(c){document.getElementById('edit_id').value=c.id;document.getElementById('edit_name').value=c.name;document.getElementById('edit_email').value=c.email||'';document.getElementById('edit_phone').value=c.phone||'';document.getElementById('edit_address').value=c.address||'';document.getElementById('editModal').classList.add('active');}</script>
</body>
</html>